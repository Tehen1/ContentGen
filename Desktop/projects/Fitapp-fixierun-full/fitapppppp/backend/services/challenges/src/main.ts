...
async function bootstrap() {
  ...
  if (environment !== 'production') {
    const config = new DocumentBuilder()
      .setTitle('FIXIE Challenges Service API')
      .setDescription('API documentation for the FIXIE challenges microservice')
      .setVersion('1.0')
      .addTag('challenges')
      .addBearerAuth()
      .build();
  ...
  logger.log(`Challenges microservice running on port ${port} in ${environment} mode`);
