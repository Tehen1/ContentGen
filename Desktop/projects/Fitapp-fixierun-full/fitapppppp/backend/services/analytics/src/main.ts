...
async function bootstrap() {
  ...
  if (environment !== 'production') {
    const config = new DocumentBuilder()
      .setTitle('FIXIE Analytics Service API')
      .setDescription('API documentation for the FIXIE analytics microservice')
      .setVersion('1.0')
      .addTag('analytics')
      .addBearerAuth()
      .build();
  ...
  logger.log(`Analytics microservice running on port ${port} in ${environment} mode`);
